...
if __name__ == "__main__":
    ...
    xold   = data.Field(domain)
    Fxold  = data.Field(domain)
    ...
    s_new.inner[R2_global < radius**2] = 0.1
    ...
    while timestep < options.nt:
        ...
        for it in range(max_newton_iters):
            ...
            xold.inner[...] = deltas.inner[...]
            operators.diffusion(options, s_old, xold, Fxold)
            def A(x, Ax):
                eps = 1.e-8
                v.inner[...] = xold.inner[...] + eps*x.inner[...]
                # f(v = s_new + eps*x)
                operators.diffusion(options, s_old, v, Fx)
                # Df(s_new)*x = A*x ~= (fv - f)/eps
                Ax.inner[...] = (Fx.inner[...] - Fxold.inner[...])/eps
